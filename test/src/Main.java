...
public class Main {
    ...
    public static void main(String[] args){
        ...
        Instant startOfYearMoment = Instant.ofEpochMilli(startOfYear);

        // и текущую дату
        Instant thisMoment = Instant.now();

        // и конечную дату нашего графика рассветов и закатов (плюс семь дней)
        Instant lastMoment = thisMoment.plusMillis(604800000);
    ...
    private static int dayOfYearFromInstant(Instant startOfYear, Instant time) {
        ...
        fromStartOfYear = fromStartOfYear/86400000;

        return (int) Math.ceil(fromStartOfYear);
    }
